...
import { createTask, createTaskOpts, editTask, taskById } from '../../redux/task_action';

const EditTask = () => {
    const params = useParams();
    const disaptch = useDispatch();
    const navigate = useNavigate();
    const { message, error, task } = useSelector((state) => state.task);

    const [title, setTitle] = useState(task?.title);
    const [status, setStatus] = useState(task?.status);
    const [priority, setPriority] = useState(task?.priority);
    const [assignTo, setAssignTo] = useState(task?.assign_to);
    ...
    const handleSubmit = () => {
        startTrans(async () => {
            await disaptch(editTask(task?.id, { title, description, priority, status, assign_to: assignTo, sub_tasks: subTasks }));
        });
        navigate('/tasks');
    };
                                ...
                                <Row key={idx}>
                                    <Col md={8}>
                                        <FormGroup>
                                            ...
                                            <Input type='text' name={'sub_tasks.' + idx + '.title'} placeholder={'SUB TASK ' + (idx + 1) + ' TITLE'} value={item.title}
                                                onChange={(e) => setSubTasks(prev => prev?.map((task, i) => i === idx ? { ...task, title: e.target.value } : task))} required
                                            />
                                        </FormGroup>
                                    </Col>
                                    <Col md={3}>
                                        <FormGroup>
                                            ...
                                            <Input type='select' name={'sub_tasks.' + idx + '.status'} value={item.status} required
                                                onChange={(e) => setSubTasks(prev => prev?.map((task, i) => i === idx ? { ...task, status: e.target.value } : task))}
                                            >
